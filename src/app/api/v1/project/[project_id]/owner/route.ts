...
export async function PUT(request: NextApiRequest, { params }: { params: { project_id: string } }) {
    const new_owner_email = request.body.email;
    if (!new_owner_email) {
    ...
    try {
        ...
        if (!project.contributors.some(contributor => contributor.email === new_owner_email)) {
            await prisma.project.update({
                where: { id: params.project_id },
                data: {
                    contributors: {
        ...
        const contributor = project.contributors.find(contributor => contributor.email === new_owner_email);
